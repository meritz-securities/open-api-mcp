...
from .safety import is_state_changing, josa
...
# 게이트웨이가 자료를 담고도 5820 을 보내는 API 가 여럿이다(실측: 담보 현황·
...
def _payload_is_empty(body: Any) -> bool:
    ...
    실측 — 담보 현황이 총자산을 담고 5820, 국내 실현손익(종목별)이 3행을 담고
...
_RESEND_OK = {
    "domestic": frozenset("134679acdfg"),   # 경고 — warn_cnfr_yn="A" 로 재전송하면 접수
...
class TokenManager:
    ...
    def get(self) -> str:
        ...
        if r.status_code != 200:
            ...
            detail = " ".join(x for x in (gw_code, gw_msg) if x) or r.text[:200].strip()
            raise MeritzError(
...
class ApiClient:
    ...
    def validate(api, params: dict) -> list[str]:
        ...
        MCP 도구뿐 아니라 **라이브러리로 직접 쓸 때도** 걸려야 한다.
        검증을 도구 계층에만 두면 meritz 패키지를 임포트해 쓰는 코드에서
        무검증으로 주문이 나간다.

        이름과 필수 여부만 보면 부족하다. **형식이 틀려도 게이트웨이가 오류를
        내지 않기 때문이다.** 실측 — 투자자매매동향에 from="2026-09-01" 을 넣으면
    ...
    def judge(api, status: int, body: Any, url: str,
        ...
        if status != 200:
            ...
            if isinstance(body, dict) and code:
                out["rsp_cd"] = code
                out["message"] = _message(body, f"게이트웨이 오류 {code}")
                _attach_sub_msg(out, body)
            else:
                # JSON 이 아닌 응답(프록시의 502 HTML 등)에는 rsp_cd 가 없다.
                # message 를 비워 두면 호출자가 "HTTP_ERROR — None" 만 보게 된다.
                head = str(body).strip().replace("\n", " ")[:200] if body else ""
                out["message"] = (
                    f"게이트웨이가 HTTP {status} 를 돌려줬습니다"
                    f"{f' — {head}' if head else ''}. "
                    f"게이트웨이·업스트림 장애일 수 있습니다. 잠시 뒤 다시 호출하세요.")
            return out
        if not isinstance(body, dict):
            return out
        ...
        if readable:
            if code in OK_CODES:
                ...
                if warned:
                    ...
                    tail = ('내용을 사용자에게 보이고 동의를 받은 뒤 warn_cnfr_yn="A" 로 '
                            "같은 주문을 다시 보내야 접수됩니다."
                            if resendable else
                ...
                if code == "0001":
                    ...
                    # 해외 실현손익 실측). 코드만 보고 no_data 를 달면 있는 자료가
                    # "자료 없음" 으로 보고된다.
                ...
                sub = out.get("rsp_sub_msg")
                if sub:
                    # 실측 — 국내 실현손익(종목별)에 시작일 > 종료일 을 넣으면
        ...
        if api["key"] not in (degraded_keys or set()):
            ...
            out["message"] = ("응답 코드를 읽을 수 없습니다. 알려진 문제 목록에 없는 "
                              "API 라 성공으로 처리하지 않습니다.")
            return out
        ...
        out["verified"] = False
        out["warning"] = ("이 API 는 응답 코드가 손상돼 오는 알려진 문제가 있습니다. "
                          "데이터를 그대로 돌려주지만 업무 성공 여부는 검증되지 않았습니다.")
        return out
