...
배포 기준은 **운영 도메인**이다. 다른 서버로 붙어야 하면 MERITZ_BASE_URL 로 지정한다.
앱키는 발급받은 환경에서만 통한다.
"""
...
def _load_env_file() -> None:
    """저장소 루트 또는 이 폴더의 .env 를 읽는다. 기존 환경변수를 덮지 않는다.

    MERITZ_NO_ENV_FILE=1 이면 읽지 않는다 — 배포 기본값을 그대로 확인할 때 쓴다.
    """
...
# MCP 서버는 에이전트가 스스로 주문을 낼 수 있으므로 조회 전용으로 시작한다.
# server.py 가 set_read_only_default(True) 로 이 값을 덮어쓴다.
_READ_ONLY_DEFAULT = "0"


def set_read_only_default(value: bool) -> None:
    global _READ_ONLY_DEFAULT
    _READ_ONLY_DEFAULT = "1" if value else "0"
...
def settings() -> Settings:
    """환경변수에서 설정을 만든다.
      ...
      MERITZ_READ_ONLY  1 이면 주문·환전을 막는다. MCP 서버는 기본이 1 이다
    ...
    return Settings(
        ...
        transport=os.getenv("MCP_TYPE", "stdio"),
